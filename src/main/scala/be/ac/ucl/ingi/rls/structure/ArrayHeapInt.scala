...
final class ArrayHeapInt(maxSize: Int) {
  ...
  final def clear(): Unit = heapSize = 0

  final def enqueue(key: Int, value: Int): Unit = {
  ...
  final def remove(key: Int, value: Int): Boolean = {
    if (heapSize == 0) false
    else {
  ...
  final def changeKey(oldKey: Int, newKey: Int, value: Int): Boolean = {
    if (heapSize == 0) false
    else {
  ...
  override def toString: String = "ArrayHeap(" + values.drop(1).take(heapSize).sorted.mkString(", ") + ")"
}
